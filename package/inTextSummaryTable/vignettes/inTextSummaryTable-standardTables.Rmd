...
The function `loadDataADaMSDTM` of the `clinUtils` package might be used to load
the data into a similar format into R.
...
of the table might depends of the study and dataset, and be subject to change.
...
```{r table-demography}
	...
	## Reformat labels:
	labelsDM <- labelVars[varsDM]
	# add unit for age
	labelsDM["AGEC"] <- labelsDM["AGE"] <- "Age (years)"
	# for sex: gender instead of sex
	labelsDM["SEXC"] <- "Gender"
	# set unit at the end of the label
	labelsDM <- sub("(.*)( \\(.+\\))(.*)", "\\1\\3\\2", labelsDM)
	labelsDM["BMIBLGR1"] <- sub(" group", "", labelsDM["BMIBLGR1"])
	labelsDM <- sub("kg/m2", "kg/m^{2}", labelsDM)
	
	# set Race in lower-case (if not already in ADaM)
	...
	labelsDM[idxDMCat] <- paste0(labelsDM[idxDMCat], ", n (%)")
...
```{r table-baselineDiseaseCharacteristics}
	...
	labelsBDC <- labelVars[varsBDC]
	labelsBDC <- sub(" group", "", sub("%FEV1", "pFEV_{1} (%)", labelsBDC))
	# add 'n (%)' in label for categorical variables
	idxBDCCat <- !colwise(is.numeric)(dataBDC[, varsBDC])
	labelsBDC[idxBDCCat] <- paste0(labelsBDC[idxBDCCat], ", n (%)")
	dataBDC$PFEVGR1 <- factor(dataBDC$PFEVGR1, 
		labels = sub("Baseline %FEV1 ", "",levels(dataBDC$PFEVGR1))
	)
	
	# create table
	getSummaryStatisticsTable(
		data = dataBDC,
		var = varsBDC, varGeneralLab = "Parameter", varLab = labelsBDC,
		colVar = "TRT01P", colTotalInclude = TRUE, colTotalLab = "All subjects",
...
```{r table-MH}
	...
	dataCM$ATC4NME <- simpleCap(tolower(dataCM$ATC4NME))
	# if missing ATC4 class, set it to: 'Other'
	dataCM$ATC4NME[dataCM$ATC4NME %in% c("", ".")] <- "Other"
	
	getSummaryStatisticsTable(
		data = dataCM,
		colVar = "TRTP", colTotalInclude = TRUE, colTotalLab = "All subjects",
		rowVar = c("ATC4NME", "CMDECOD"), 
		rowVarLab = c(ATC4NME = "ATC class", CMDECOD = "Generic term, n(%)"),
		# include total across generic terms and across ATC4 classes
		rowVarTotalInclude = c("ATC4NME", "CMDECOD"), 
		...
		filterFct = function(summaryTable){
			ddply(summaryTable, "ATC4NME", function(xATC4)
				ddply(xATC4, "CMDECOD", function(xTerm)
					if(subset(xTerm, grepl("All subjects", TRTP))$statN >= 2)	xTerm
				)
			)
		},
		emptyValue = 0,
		title = toTitleCase(paste("Prior and concomitant therapies reported in at least",
			"2 subjects overall by ATC class (level 4) and generic term (full analyis set)"
		)),
...
```{r table-spirometry}
	...
	dataPFEV1 <- convertVarToFactor(dataPFEV1, 
		var = c("TRTP", "AVISIT"), 
	...
	getSummaryStatisticsTable(
		...
		stats = statsEff,
		labelVars = labelVars,
		title = paste("Table: %pFEV_{1}", 
			toTitleCase("actual value and changes from baseline per time point (full analysis set)"				
		)),
		file = file.path("tables_CSR", "Table_pFEV1ChangeFromBaseline.docx")
	)
...
```{r table-summaryTable}
	...
	dataTEAE <- ddply(dataTEAE, c("USUBJID", "TRTA"), function(x)
		cbind.data.frame(x, 
			WORSTINT = with(x, ifelse(AESEVN == max(AESEVN), simpleCap(tolower(AESEV)), NA_character_))
	))
...
```{r tableLab}
	...
	dataLBWCWithAbn$PARCAT1 <- factor(dataLBWCWithAbn$PARCAT1, 
		labels = simpleCap(tolower(levels(dataLBWCWithAbn$PARCAT1)))
	)
...
```{r PK}
	...
	statsPK <- dlply(dataPK, "PARAM", function(dataParam)
		...
		# mean (CV %) for others
			...
			list(bquote(paste0(
				roundHalfUpTextFormat(statMean, .(nDecBase + 1)), 
				" (", roundHalfUpTextFormat(statCV, .(nDecBase + 1)), ")"
			)))
		}
	)
	...
	getSummaryStatisticsTable(
		data = dataPK,
		rowVar = c("PARCAT1", "PARAM"), colVar = c("TRTA", "AVISIT"),
		var = "AVAL",
