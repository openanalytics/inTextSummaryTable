...
```{r table-TEAE}
	...
	dataAE <- subset(dataAll$ADAE, SAFFL == "Y")
...
#	dataAEMulEvents <- subset(dataAE %>% group_by(SUBJID, AEDECOD) %>% dplyr::summarise(n = n(), nWI = toString(WORSTINT)), n > 1)
#	join(dataAE, dataAEMulEvents)	
	# use AETOXGR and not WORSTINT for ordering (no WORSTINTN) available
	dataAE <- convertVarToFactor(
		data = dataAE,
		var = c("TRTA", "AESEV"), varNum = c("TRTAN", "AESEVN")
	)
	
	# include columns with worst-case scenario (only one record if multiple with same severity)
	dataAEWC <- ddply(dataAE, c("AEDECOD", "USUBJID", "TRTA"), function(x){
#		if(nrow(x) > 1)	print(x[, c("AEDECOD", "USUBJID", "TRTA", "AESEV")])
...
#		rowOrderTotalFilterFct = function(x) subset(x, TRTA == "Total"),
		...
		title = paste("Table: Severity of Treatment-emergent Adverse Events by Preferred Term",
			"(Safety Analysis Set)"
		),
...
```{r dataTotalSC}
	...
	dataTotalSC <- subset(dataAll$ADSL, SAFFL == "Y" & grepl("Part 1", ARM))
	dataTotalSC <- addTotalScCategory(dataTotalSC)# build category with all s.c. doses
	# TRTA in general, TRTP for PK
	dataTotalSC$TRTA <- dataTotalSC$TRTP <- dataTotalSC$TRT01A <- formatTreatment(x = dataTotalSC$TRT01A, separatedRow = TRUE)

```

```{r tableAE-incidence-dataPreparation}

	dataAE <- subset(dataAll$ADAE, SAFFL == "Y" & grepl("Part 1", ARM) & TRTEMFL == "Y")
	
	# build category with all s.c. doses
	dataAE <- addTotalScCategory(dataAE, treatVar = "TRTA")
	
	dataAE$TRTA <- formatTreatment(x = dataAE$TRTA, separatedRow = TRUE)

	getAdverseEventsTable <- function(...)
		getSummaryStatisticsTable(
			data = dataAE,
			rowVar = c("AEBODSYS", "AEDECOD"),
			colVar = "TRTA",
			dataTotal = dataTotalSC,
			colTotalInclude = TRUE,
			stats = list(expression(paste0(roundCustomText(statN, 0), " (", roundCustomText(statPercN, 1), ")"))),
			labelVars = labelVars,
			rowVarTotalInclude = c("AEBODSYS", "AEDECOD"), rowTotalLab = "Any TEAE",
			rowVarLab = c('AEBODSYS' = "TEAE by SOC and Preferred Term,\nn (%)"),
			footer = c(
				"N=number of subjects with data; n=number of subjects with this observation",
				"Denominator for percentage calculations = the total number of subjects per treatment group in the safety population, within the analysis period."
			),
			rowOrder = "total", 
			rowOrderTotalFilterFct = function(x) subset(x, TRTA == "Total"),
			...
