...
```{r table-MH}
	...
	getSummaryStatisticsTable(
		...
		stats  = getStats("n (%)"),
		# TODO: add possibility to specify directly columns to order by?
#		rowOrder = function(summaryTable){
#			dataTotal <- subset(summaryTable, grepl("All subjects", TRTP))
#			dataTreatment <- subset(summaryTable, grepl("treatmentX", TRTP))
#			dataAll <- merge(dataTotal, dataTreatment, by = "CMDECOD", suffixes = c(".total", ".treated"))
#			dataAll[order(dataAll[, c("statPercN.total", "statPercN.treated")]), "CMDECOD"]
#		},
