...
To include categories which are not present in the data, `var` should be a
factor and these categories should be specified within its levels.
...
statistics for empty categories** within the row/column. These categories are
specified via additional **levels in a factor variable**.
...
The name of the statistic are include or not 
...
Please note that the content of the table might depends of the study (see SAP).
...
```{r useCase-demoTable}
	...
	# data of interest
	dataDMSAS <- subset(dataAll$ADSL, SAFFL == "Y")
	# variables of interest
	varsDM <- c("AGE", "BMIBL", "BMIBLGR1", "RACE", "SEXC")
	
	# sort BMI groups from lowest to highest categories
	dataDMSAS$BMIBLGR1 <- with(dataDMSAS, reorder(BMIBLGR1, BMIBLG1N))
	
	# order SEX, RACE and treatment categories according to their numeric variable
	dataDMSAS <- convertVarToFactor(
		data = dataDMSAS,
		var = c("SEXC", "RACE", "TRT01A"),
		varNum = c("SEXN", "RACEN", "TRT01AN")
	)
	
	# for sex: use 'Gender' and not 'Sex' label
	labelVars["SEXC"] <- "Gender"
	
	# statistics of interest
	statsNum <- getStats("median (range)")
	statsCat <- getStats("n (%)", includeName = FALSE)
	varsDMIsNum <- colwise(is.numeric, varsDM)(dataDMSAS)
	...
	getSummaryStatisticsTable(
		...
		# column
		colVar = "TRT01A", colTotalInclude = TRUE,
		# statistics
		stats = statsDM,
		statsGeneralLab = "",
		labelVars = labelVars,
