...
The example datasets from the _Pelican_ study included in the
`inTextSummaryTable` package itself are used for demonstration in the
vignette.  
...
```{r stats-digits}
	...
	getSummaryStatisticsTable(
		...
		var = c("AGE", "HEIGHT"),
		stats = list(
			AGE = list(Median = expression(roundCustom(statMedian, 1))),
			HEIGHT = list(Median = expression(roundCustom(statMedian, 2)))
		)
	)
...
in case of 0.5. The `roundCustom` function rounds to the closest digits.
...
```{r stats-digits-complex}
	...
	statsDMNum <- function(digitsMin)
		list('Median (range)' = 
			bquote(paste0(roundCustom(statMedian, .(digitsMin)+1), 
				" (", roundCustom(statMin, .(digitsMin)), ",", roundCustom(statMax, .(digitsMin)),
				")")	
		)
	)
...
```{r rowOrder-specific}
	...
	getSummaryStatisticsTable(
		...
		rowVar = c("AESOC", "AEDECOD", "AESEV"),
		rowVarInSepCol = "AESEV",
		rowOrder = list(AESOC = "total", 'AEDECOD' = "alphabetical", AESEV = "auto"),
		stats = getStats("n (%)"),
		labelVars = labelVars
	)
...
```{r summaryTable-PP-rowVarWithLabel-rowVarLab-superscript}
	...
	getSummaryStatisticsTable(
		...
		stats = list(expression(paste0(roundCustom(statMean, 1), "\n(", roundCustom(statSE, 2),")"))),
...
```{r countTable-AE-filterFct-rowVar1}
	...
	getSummaryStatisticsTable(
		...
		labelVars = labelVars,
		file = file.path("tables", "Table_TEAE_filterFct.docx")
	)
	...
	getSummaryStatisticsTable(
		...
		filterFct = function(x)
			ddply(x, "AEDECOD", function(xTerm){
				...
				if(any(xTermTreatment$statPercN >= 10))	xTerm		
			}),
		labelVars = labelVars
	)
		...
		list(
			bquote(
				ifelse(statMean == 0, 0, paste0(
						roundCustom(statMean, .(nMaxDec + 1)), " (",
						roundCustom(statCVPerc, .(nMaxDec)), ")"
					)
				)
			)
		)
