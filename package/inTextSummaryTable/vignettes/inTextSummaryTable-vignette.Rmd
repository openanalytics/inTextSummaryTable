...
```{r summaryTable-PP-rowVarMultiple}
	...
	getSummaryStatisticsTable(
		...
		rowPadBase = 0.4*72 # 4 inches in points
	)
...
specific row variable, this variable is specified in the
the `rowVarTotalInclude` variable.

The label for the total is customized via the `rowTotalLab` parameter.

For example, the total across all adverse events row variables is included by
specifying the first row variable.
...
```{r countTable-LB-rowTotalInclude}
	...
	getSummaryStatisticsTable(
		...
		title = "Table: Adverse Events",
		dataTotal = dataTotal,
		rowVarTotalInclude = "AEHLGT", rowTotalLab = "Any adverse events",
		stats = list(expression(paste0(statN, " (", round(statPercN, 1), ")"))),
		footer = "Statistics: N (%)"
	)
...
The total for **each sub-group of the row variable(s)** is now by specifying
all row variables in `rowVarTotalInclude`.
...
```{r countTable-LB-rowSubtotalInclude}
	...
	getSummaryStatisticsTable(
		...
		title = "Table: Adverse Events",
		dataTotal = dataTotal,
		rowVarTotalInclude = c("AEHLGT", "AEHLT", "AELLT"),
		stats = list(expression(paste0(statN, " (", round(statPercN, 1), ")"))),
		footer = "Statistics: N (%)"
	)
...
are retained (ordered alphabetically if not specified), e.g. to use the
       ordering based on the corresponding numeric variable  in the dataset
       ('AEHLGTCD' for 'AEHLGT')
...
```{r countTable-LB-rowOrder-common}
	...
	getSummaryStatisticsTable(
		...
		dataTotal = dataTotal,
		rowVarTotalInclude = c("AEHLT", "AELLT"),
		stats = list(expression(paste0(statN, " (", round(statPercN, 1), ")"))),
		footer = "Statistics: N (%)"
	)
...
```{r mixedTable-SL}
	...
	getSummaryStatisticsTable(
		...
		varGeneralLab = "Characteristic", varSubgroupLab = "Class",
		labelVars = labelVars,
