...
```{r summaryTable-PP-medianMinMax}
	...
	getSummaryStatisticsTable(
		...
		labelVars = labelVars,
		stats =  list(
			Mean = expression(formatC(Mean)),
			Median = expression(formatC(Median))
		),
...
```{r summaryTable-PP-medianMinMax-statsLayoutRowl}
...
```{r countTable-LB-rowOrder-customFunction}
	...
	getSummaryStatisticsTable(
		...
		rowVar = "AEHLT",
		rowOrder = function(x){
			totalPerCat <- daply(x, "AEHLT", function(y) sum(y$N))# count across treatments
			totalForTreatOnly <- daply(x, "AEHLT", function(y)
				subset(y, TRTP == "treatmentX")$N
...
```{r mockUpTables}
	...
	getSummaryStatisticsTable(
		...
		labelVars = labelVars,
		rowPadBase = 10,
		stats = list(expression(paste0("[X]"))),
