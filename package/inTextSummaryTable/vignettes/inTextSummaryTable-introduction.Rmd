...
[here](inTextSummaryTable-createTables.html).
...
if a record fulfillss a specific criteria.
...
The name of such variable typically ends with **'FL'** in CDISC-compliant _ADaM_
or _SDTM_ dataset.
...
```{r flag-variables}
    ...
    # does the subject belong to the safety analysis set?
    dataSL$SAFFL
...
Please note that **missing records for the variable are filtered**, so the
...
By default, the number of decimals is based on the rules specified in the _Mock
Standard SAP **Phase 2/3**_ (version 1.0)
...
According to the Mock SAP, the counts of number of subjects is reported without
decimals. The number of decimals for the percentages is specified below:

```{r nDecimals-catVar-SAP-phase23, echo = FALSE, fig.cap = "Standard Layout for Frequency Tabulations of Categorical Variables<br>"}
...
The number of decimals for individual values is based on the Mock SAP (see
tables below) unless the number of digits in the database is lower. In this
case, the resolution of the specified values is used.

```{r nDecimals-numVar-SAP-phase23, echo = FALSE, fig.cap = "Standard Layout for Descriptive Statistics of Continuous Variables<br>"}
...
3. The actual number of decimals for each statistic is extracted (based on the
   Mock TLFs) by adding to the 'base' number of decimals:
...
Notes: by default the `round` function in R rounds the number to the even number
in case of 0.5. The `roundHalfUpTextFormat` function rounds to the closest
digit.
...
* total reported in the **column header** Typically: total number of subjects
...
|---|---|---|---| | Total in the column header | Yes by default <br/> removed if
`colHeaderTotalInclude = FALSE`| `dataTotal` | `data` for table content <br/>
`dataTotalCol` for total column| | Total for the percentage | Only if percentage
requested in `stats` | `dataTotalPerc` | `dataTotal` for table content <br/>
`dataTotalCol` for total column <br/> (for 'total' if specified as a list)| |
Total across rows | Not by default <br/> for specified row variable with
`rowVarTotalInclude` | `dataTotalRow`  | `data` for table content <br/>
`dataTotalCol` for total column <br/> (for 'total' if specified as a list) | |
Total across columns | Not by default <br/> only if `colTotalInclude = TRUE` |
`dataTotalCol` | `data` |
...
For example, in the table of shift of worst-case laboratory abnormalities: the
total number of subjects for the computation of the percentage are extracted
based on the number of subjects with available post-dose measurements **per
laboratory parameter**.

```{r rowVarTotalPerc}

	dataLB <- subset(dataAll$ADLBC, SAFFL == "Y" & PARAMCD %in% c("CL", "ALB"))
	
	dataLBWC <- subset(dataLB, AVISIT == "Worst-case post-baseline")

	# data considered for the total
	dataLBPostBaseline <- subset(dataLB, ! grepl("Baseline|End of Treatment", AVISIT))
#	
...
For example, in the worst-case treatment-emergent adverse events table, the
total per SOC and across SOCs should count the patient once per severity class.
...
```{r dataTotalRow}
	...
	dataTEAEInterest <- subset(
		dataAll$ADAE, 
        SAFFL == "Y" & TRTEMFL == "Y" &
		grepl("Injury, poisoning and procedural complications|Nervous system disorders", AESOC, ignore.case = TRUE)
	)
	...
	kable(ddply(dataTEAEInterestWC[, c("USUBJID", "TRTA", "AESOC", "AEDECOD", "AESEV")], "USUBJID"), split.tables = Inf)
...
For example, the patient: '202-4903-03' present a moderate severity for
headache, and mild severity for the chest injury and fall, but is only counted
in the once in the table total.
...
the function `knitPrintListObjects` of the `clinUtils` package can be used.
...
#		footer = c(
#			"(1) Each compound is labelled according to its compound ID.",
	...
	getSummaryStatisticsTable(
		...
		filterFct = function(x)
			ddply(x, "AEDECOD", function(xTerm) {
				xTermTreatment <- subset(xTerm, grepl("High dose", TRTA))
...
#	subset(xTerm, grepl("High dose", TRTA))
#	# keep all records (placebo + treatment) if percentage if higher than 20%:
#	# if no 'else' condition, nothing (NULL) is returned:
#	if(subset(xTerm, grepl("High dose", TRTA))$statPercN >= 20)	xTerm
#	# across all AE terms:
#	ddply(x, "AEDECOD", function(xTerm)
#		if(subset(xTerm, grepl("High dose", TRTP))$statPercN >= 20)	xTerm
#	)
