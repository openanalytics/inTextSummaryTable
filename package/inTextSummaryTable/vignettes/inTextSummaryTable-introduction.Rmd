...
The set of summary statistics can be combined and the format customized
via the `stats` parameter. This parameter is an list of `expression` of the
...
The `getStats` function wraps up a common default set of statistics, that are passed
directly to the `stats` function.

For a continuous variable, it is **recommended to pass also the
variable to the `getStats` function** to get
appropriate number of standard number of decimals 
...
A set of extra statistics, not available in the default statistics, can
...
which is used as 'base' number of decimals considered for the summary statistics ($n_{dec, base}$)
3. The actual number of decimals for each statistics is extracted (based on the Mock TLFs) with:
...
```{r getMaxNDecimals}
	...
	# based on pre-define rule, this metric should be displayed with 1 decimal:
	...
	# extract sets of statistics of interest
	getSummaryStatisticsTable(
		data = dataSL,
		var = c("AGE", "PFEVBL"),
		stats = list(
...
The `roundCustomText` function rounds to the closest digits.
...
```{r rowOrder-common}
	...
	getSummaryStatisticsTable(
		...
		rowVarTotalInclude = c("AESOC", "AEDECOD"),
		colVar = "TRTP", 
		rowOrder = "total", colTotalInclude = TRUE, colTotalLab = "Number of subjects",
		stats = getStats("n (%)"),
		labelVars = labelVars
	)
...
A subject presenting the **same event in multiple column(s) are counted once**
...
If the data is loaded into R with the `loadDataADAM` function, each column of the
dataframe contains a specific label in its 'label' attribute.

However, if this label is lost (e.g. if the object is subsetted), 
labels (for variable, row and column) can be specified via the
`labelVars` parameter, or a specific '[parameter]Lab' parameter (e.g.
