...
of summary statistics for a variable of interest.
...
The variable (should be **numeric**) to summarize is specified via the `var`
parameter.
By default, an
...
Grouping for the rows can be specified via multiple `rowVar`, which should be
...
The label used for the `rowVar` parameter can be automatically extracted by the
labels contained in the _SAS_ dataset, by specifying the `labelVars` parameter.
...
In this case, no `var` variable are requested.
...
for the count table.
For example, the counts of subjects and records for adverse events are
reformatted as: 'N/m (%)' with the `stats` parameter, and the adverse event term
are grouped according to the `r labelVars["AEHLGT"]` ('AEHLGT) and
