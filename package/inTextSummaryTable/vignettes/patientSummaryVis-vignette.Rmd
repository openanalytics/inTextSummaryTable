...
The package `inTextSummaryTable` contains functionalities to create summary
statistics table, format them as in-text table, and export to a`docx`
document.
...
usually loaded a _SAS_ data file (`sas7bdat` format).
The label of the variables stored in the `SAS` datasets is also used
for title/captions. 

Example datasets included in the `inTextSummaryTable` package are used in this
vignette.
Note that the `loadDataADaMSDTM` function of the `glpgUtilityFct` package can
be used to load the custom your custom `SAS` dataset(s) of interest.
...
The main function is `getSummaryStatisticsTable`, creates a in-text table
of summary statistics for a variable of interest and for specific
...
By default, an entire set of summary statistical metrics are computed, see
...
If the specified variable of interest (`var`) is not numeric (`character` or
...
The laboratory parameters are grouped by category, and the visit is indicated
in the rows: `PARCAT1`, `PARCAT2` and `AVISIT` are specified in `rowVar`.
The treatment is reported in the column: `TRTP` specified in `colVar`.
...
The statistics are reformatted as 'N (%)' by using the `stats` parameter.

```{r countTable-LB-custom}
...
```{r countTable-AE}
	...
	# create summary table
	summaryTableAE <- getSummaryStatisticsTable(
		data = dataAE,
		rowVar = c("AESOC", "AELLT"),
		colVar = "TRTP",
		var = NULL, #"AESEV", 
		labelVars = labelVars,
		rowPadBase = 10,
		title = "Table: Adverse events",
		stats = list(expression(paste0(N, " (", round(Perc, 1), ")"))),
		dataTotal = dataTotal,
		footer = "Statistics: N (%)",
		file = "summaryStatisticsTable_AE.docx",
	)
	summaryTableAE
